...
function renderAurora(seed, width, height) {
	...
	if (light) {
		baseGrad.addColorStop(0, hsla(palette[0].h, 38, 97, 1));
		baseGrad.addColorStop(0.35, hsla(palette[0].h, 34, 95, 1));
		baseGrad.addColorStop(0.7, hsla(palette[1].h, 30, 93, 1));
		baseGrad.addColorStop(1, hsla(palette[2].h, 26, 91, 1));
	} else {
	...
	blobCtx.globalCompositeOperation = light ? "multiply" : "screen";
	...
	for (let i = 0; i < blobCount; i++) {
		...
		const alpha = light ? 0.22 + rng() * 0.14 : 0.14 + rng() * 0.12;
	...
	ctx.globalCompositeOperation = light ? "overlay" : "soft-light";
	const sweep = ctx.createLinearGradient(0, height * 0.02, width, height * 0.98);
	if (light) {
		sweep.addColorStop(0, hsla(palette[2].h, 45, 72, 0.05));
		sweep.addColorStop(0.25, hsla(palette[2].h, 50, 68, 0.08));
		sweep.addColorStop(0.45, hsla(palette[0].h, 70, 58, 0.12));
		sweep.addColorStop(0.55, hsla(palette[1].h, 75, 55, 0.1));
		sweep.addColorStop(0.75, hsla(palette[0].h, 65, 62, 0.07));
		sweep.addColorStop(1, hsla(palette[2].h, 40, 74, 0.05));
	} else {
...
function renderIdenticon(seed) {
	...
	const colors = light
		? [
				`hsl(${seed % 360}, 58%, 42%)`,
				`hsl(${(seed * 3) % 360}, 52%, 48%)`,
				`hsl(${(seed * 7) % 360}, 38%, 36%)`,
			]
		: [
				`hsl(${seed % 360}, 58%, 48%)`,
	...
	let shapes = `<rect width="${view}" height="${view}" fill="hsl(${(seed + 120) % 360}, ${light ? 28 : 22}%, ${light ? 94 : 10}%)"/>`;
...
function renderFlow(seed, width, height) {
	...
	const hue = seed % 360;
	const light = isLightTheme();
	ctx.fillStyle = light ? `hsl(${hue}, 28%, 95%)` : `hsl(${hue}, 22%, 7%)`;
	ctx.fillRect(0, 0, width, height);
	...
	for (let p = 0; p < count; p++) {
		...
		const lineLight = light ? 38 + rng() * 8 : 58;
		const lineAlpha = light ? 0.28 + rng() * 0.2 : 0.22 + rng() * 0.18;
		ctx.strokeStyle = `hsla(${lineHue}, 62%, ${lineLight}%, ${lineAlpha})`;
